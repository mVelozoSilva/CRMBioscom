...
<head>
    ...
    <title>Editar Producto - Bioscom CRM</title>
    <style>
    ...
    <div class="main-content">
        ...
        <form action="{{ route('productos.update', $producto->id) }}" method="POST">
            ...
            <div class="form-group">
                ...
                <input type="number" step="0.01" id="precio_neto" name="precio_neto" value="{{ old('precio_neto', $producto->precio_neto) }}" required min="0">
                @error('precio_neto') <span class="error-message">{{ $message }}</span> @enderror
            </div>

            <div class="form-group">
                <label for="categoria">Categoría:</label>
                <input type="text" id="categoria" name="categoria" value="{{ old('categoria', $producto->categoria) }}">
                @error('categoria') <span class="error-message">{{ $message }}</span> @enderror
            </div>

            <div class="form-group">
                <label for="imagenes">Imágenes (JSON de URLs):</label>
                <textarea id="imagenes" name="imagenes" placeholder='["url_imagen1.jpg", "url_imagen2.png"]'>{{ old('imagenes', json_encode($producto->imagenes)) }}</textarea>
            ...
            <div class="form-group">
                ...
                <textarea id="accesorios" name="accesorios" placeholder='["Accesorio A", "Accesorio B"]'>{{ old('accesorios', json_encode($producto->accesorios)) }}</textarea>
            ...
            <div class="form-group">
                ...
                <textarea id="opcionales" name="opcionales" placeholder='["Opción X", "Opción Y"]'>{{ old('opcionales', json_encode($producto->opcionales)) }}</textarea>
